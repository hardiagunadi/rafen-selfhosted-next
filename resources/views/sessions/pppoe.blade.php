        ...
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center" style="gap:.5rem;">
                <div class="btn-group btn-group-sm">
                    <a href="{{ route('sessions.pppoe') }}" class="btn btn-outline-success active">Aktif</a>
                    <a href="{{ route('sessions.pppoe-inactive') }}" class="btn btn-outline-danger">Tidak Aktif</a>
                </div>
    ...
    (function () {
        ...
        function syncAllSessions(options) {
            ...
            return fetch(refreshAllUrl, {
                ...
                .then(function (data) {
                    ...
                    countdown = 60;
                    if (showSuccess) {
                        var msg = 'PPPoE: <strong>' + data.ppp_online + '</strong> sesi aktif';
        ...
        document.getElementById('btn-refresh-router').addEventListener('click', function () {
            ...
            fetch(url, { method: 'POST', headers: { 'X-CSRF-TOKEN': csrfToken, 'X-Requested-With': 'XMLHttpRequest',
                'Content-Type': 'application/json' }, body: JSON.stringify({ service: 'pppoe' }) })
                .then(function (r) { return r.json(); })
                .then(function (data) {
                    if (dtTable) dtTable.ajax.reload(function (json) { updateTotal(json.recordsTotal); }, false);
                    countdown = 60;
