...
export class Whatsapp {
        ...
        sock.ev.process(async (events) => {
          if (events["connection.update"]) {
            ...
            if (connection === "close") {
              ...
              const codeNames: Record<number, string> = { 401: 'loggedOut', 408: 'connectionLost', 411: 'multideviceMismatch', 428: 'connectionClosed', 440: 'connectionReplaced', 500: 'badSession', 503: 'unavailableService', 515: 'restartRequired' };
              console.log(`[session:${sessionId}] connection closed — code=${code} (${codeNames[code] || 'unknown'}) isLoggedOut=${isLoggedOut}`);
              let retryAttempt = this.retryCount.get(sessionId) ?? 0;
              let shouldRetry = false;
              if (!isLoggedOut && !isConnectionReplaced && retryAttempt < 10) {
                shouldRetry = true;
              }
              if (shouldRetry) {
                console.log(`[session:${sessionId}] retrying (attempt ${retryAttempt + 1}/10)...`);
                retryAttempt++;
                ...
                if (isLoggedOut) {
                  console.log(`[session:${sessionId}] LOGGED OUT — credentials cleared`);
                  // User logged out from phone — clear credentials
                  this.deleteSession(sessionId);
                } else {
                  console.log(`[session:${sessionId}] retry exhausted — socket stopped, credentials kept`);
                  // Retry exhausted due to network issues — stop socket but keep credentials
                  session?.sock.end(undefined);
                  this.sessions.delete(sessionId);
                }
                this.callback.get(CALLBACK_KEY.ON_DISCONNECTED)?.(sessionId, code);
                options.onDisconnected?.(sessionId, code);
              }
            }
            if (connection == "open") {
              this.retryCount.delete(sessionId);
  ...
  /**
   * Close session socket without logout and keep credentials intact.
   */
